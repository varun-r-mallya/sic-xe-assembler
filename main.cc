...
public:
  explicit InstructionValidator(const std::string &instruction)
      : instruction_(instruction) {}

  std::string isValid() {
    int size_of_instruction = num_of_bytes();
    switch (size_of_instruction) {
    case 1:
      validate_case(1);
      break;
    case 2:
...
private:
  ...
  int num_of_bytes() const {
    std::size_t size_of_string = instruction_.size();
    if (size_of_string % 2) {
      return -1;
    }
  ...
  void validate_case(int size_of_instruction_) {
    ...
    if (formatting.valid) {
      ...
      if (formatting.fmt1 == size_of_instruction_) {
        ...
        validity_ = false;
        error_message =
            "Invalid number of operands for " + name_of_instruction_;
      }
    } else {
      validity_ = false;
